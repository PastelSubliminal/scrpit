...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (sel == 0)
            ...
            cm.sendNext("接下来告诉你如何拣取物品，当你击退怪物时，会有机会掉落宝物以及金币，当地上有物品时，按下#bZ#k 或是 数字键盘上的 #b0 来捡取物品。");
        else if (sel == 3)
            ...
            cm.sendNext("你想要知道这个岛屿吗？ 这里是彩虹岛，这座岛屿浮在天空上。由于浮在天空上，强大的怪物们无法靠近。这里非常和平，非常适合新手。");
        else if (sel == 6)
            cm.sendNext("你想成为#b战士#k？ 摁...那我建议你到维多利亚港，寻找一个叫做#r勇士部落#k的战士村庄以及去找寻#bDances with Balrog#k。 他会教你如何成为一个战士。 喔对了，有件很重要的事，你必须达到等级10才能成为战士！");
        else if (sel == 7)
