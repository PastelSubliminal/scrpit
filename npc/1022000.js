		...
		if (status == 0) {
			if (cm.getJob() == 0) {
				if (cm.getLevel() >= 10)
					...
					cm.sendOk("你还不能转制成#r战士，转职成#r战士最少需要10级")
					cm.dispose();
				}
			} else {
                ...
                if (cm.getQuestStatus(100101) == 2) {
					...
					cm.dispose();
                                    ...
                                    if(cm.haveItem(4031380,1)){
                                        cm.sendOk("你居然带来了#v4031380#，我们进入正题吧。现在我的分身已经出现了，和他决斗吧。赢了可以获得#b黑符#k一张\r\n(#v4031380#已经删除，如果这次失败，请再去三转教官获取！)");
			...
			cm.sendYesNo("你是否想转职成为 #r" + jobName + "#k?");
		} else if (status == 23) {
			cm.changeJob(job);
