...
function action(mode, type, selection) {
	...
	if (status == 0) {
		cm.sendSimple("哈啰 我是雷娜丽！ 很高兴为您服务～#k\r\n#b#L0#我想要知道家族联盟是什么？#l\r\n#L1#我要怎么建立家族联盟呢？#l\r\n#L2#我想要建立家族联盟#l\r\n#L3#我想要新增更多的家族到联盟#l\r\n#L4#我想要解散家族联盟#l");
	} else if (status == 1) {
		choice = selection;
	    if (selection == 0) {
		    cm.sendOk("家族联盟就是让两方的家族成员可以聊天做一些有趣的事情。");
			cm.dispose();
		} else if (selection == 1) {
			cm.sendOk("为了成立家族联盟，两个家族的会长需要组队，然后这个组队里的队长就会选为家族联盟的会长。");
			cm.dispose();
		} else if(selection == 2) {
			if (cm.getPlayer().getParty() == null || partymembers == null || partymembers.size() != 2 || !cm.isLeader()) {
				cm.sendOk("你不能创建一个家族联盟，直到你找到另一个家族。"); //Not real text
				cm.dispose();
			} else if (partymembers.get(0).getGuildId() <= 0 || partymembers.get(0).getGuildRank() > 1) {
				cm.sendOk("你不能创建一个家族联盟，直到你有自己的家族。");
				cm.dispose();
				...
				if (gs.getAllianceId() > 0) {
					...
					cm.sendOk("你的成员已经和其他家族结为同盟了。");
					cm.dispose();
				} else if (cm.partyMembersInMap() < 2) {
					cm.sendOk("请确保其他成员在同张地图上。");
					cm.dispose();
				} else
                cm.sendYesNo("哦，你有兴趣创建一个家族联盟？");
			}
		} else if (selection == 3) {
			if (cm.getPlayer().getGuildRank() == 1 && cm.getPlayer().getAllianceRank() == 1) {
			    ...
			    cm.sendOk("只有家族联盟长可以扩大联盟。");
				cm.dispose();
			}
		} else if(selection == 4) {
			if (cm.getPlayer().getGuildRank() == 1 && cm.getPlayer().getAllianceRank() == 1) {
				cm.sendYesNo("你真的想要解散家族联盟？？");
			} else {
				cm.sendOk("只有家族联盟长才可以解散。");
				cm.dispose();
			}
		}
	} else if(status == 2) {
	    if (choice == 2) {
		    cm.sendGetText("现在请输入你想要的家族联盟名称 (最大字元限制. 12 个字)");
		} else if (choice == 3) {
			if (cm.getPlayer().getGuildId() <= 0) {
				cm.sendOk("你不能增加不存家族联盟。");
				cm.dispose();
			} else {
				if (cm.addCapacityToAlliance()) {
					cm.sendOk("你成功增加了家族联盟容量。");
				} else {
					cm.sendOk("很抱歉，由于你的家族联盟容量已经满了，所以不能再扩充。");
				}
				cm.dispose();
			}
		} else if (choice == 4) {
	    ...
	    cm.sendYesNo("这个 #b"+ guildName + "#k 是你想要的家族联盟名字吗？？");
	} else if (status == 4) {
